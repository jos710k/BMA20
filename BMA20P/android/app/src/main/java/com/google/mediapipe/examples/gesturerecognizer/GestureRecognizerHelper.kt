...
class GestureRecognizerHelper(
    ...
    private fun returnLivestreamResult(
        ...
        // Detectar gesto ✌️ (por ejemplo, 'Victory')
        val gestures = result.gestures()
        if (gestures.isNotEmpty()) {
            val topGesture = gestures[0].firstOrNull()
            if (topGesture != null && topGesture.categoryName().equals("Victory", ignoreCase = true)
            ) {
                val now = SystemClock.uptimeMillis()

                if (!capturePending && now - lastGestureTime > 5000) {
                    capturePending = true
                    lastGestureTime = now

                    // Ejecuta después de 3 segundos
                    android.os.Handler(context.mainLooper).postDelayed({
                        gestureRecognizerListener?.onVictoryGestureDetected()
